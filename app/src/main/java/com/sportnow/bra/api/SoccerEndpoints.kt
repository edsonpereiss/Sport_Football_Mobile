package com.sportnow.bra.api

import androidx.lifecycle.LiveData
import com.sportnow.bra.api.callbacks.*
import retrofit2.Call
import retrofit2.http.GET
import retrofit2.http.Path

interface SoccerEndpoints {
    companion object {
        private const val fixtures = "fixtures.localTeam,fixtures.visitorTeam,fixtures.league,fixtures.league.country"
        private const val upcoming = "upcoming.localTeam,upcoming.visitorTeam,upcoming.league,upcoming.league.country"
        private const val results = "results.localTeam,results.visitorTeam,results.league,results.league.country"
    ...
    @GET("leagues?api_token=$API_KEY&include=country:order(code|desc)")
    ...
    @GET("seasons/{id}?api_token=$API_KEY&include=upcoming$limit,$upcoming,results$limit,$results")
    fun seasonMatches(@Path("id") seasonId: Long): Call<SeasonMatchesCallback>
    ...
    @GET("topscorers/season/{seasonId}?api_token=$API_KEY&include=goalscorers$playerLimit:order(goals|desc),goalscorers.player,assistscorers$playerLimit:order(assists|desc),assistscorers.player,cardscorers$playerLimit:order(yellowcards|desc),cardscorers.player")
